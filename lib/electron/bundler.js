...
function isInstalled (bundlerName) {
  return fs.existsSync(appPath.resolve.node_modules(`electron-${bundlerName}`))
}
  ...
  if (bundlerName === 'packager') {
    if (isInstalled('packager')) {
      const
        ...
        pkg = require(appPath.resolve.node_modules(`electron-${bundlerName}/package.json`))
...
module.exports.getBundler = function (bundlerName) {
  return require(appPath.resolve.node_modules(`electron-${bundlerName}`))
}
